...
object TableColumnsSpec extends DefaultRunnableSpec {
  ...
  def assertFragmentSqlEqual(left: Fragment, right: String) = assert(left.query.sql)(equalTo(right))

  def assertStringEqual(left: String, right: String) = assert(left)(equalTo(right))
